...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		//evicted frame page from physical memory
		pgtbl_entry_t *evicted = coremap[frame].pte;

		//check if frame holds vpage - always should be true else an error
		if (evicted->frame & PG_VALID) {
			// if so, write to swap if it is modified from what's in swap (or could be first reference (doublecheck))
			if (evicted->frame & PG_DIRTY || !(evicted->frame & PG_REF)) {
				//try try try
				//int swap_off = swap_pageout(evicted->frame >> PAGE_SHIFT, evicted->swap_off)
				int swap_off = swap_pageout(frame, evicted->swap_off);
				if (swap_off == INVALID_SWAP) {
					return INVALID_SWAP;
				}
				evicted->swap_off = swap_off;
				//renewed on disk, so not modified (not dirty)
				evicted->frame &= ~PG_DIRTY;
      			evict_dirty_count += 1;
			} else {
				evict_clean_count += 1;
			}
            //not in physical memory
            evicted->frame &= ~PG_VALID;
            //on swap since on disk
      		evicted->frame |= PG_ONSWAP;
            //referenced (doublecheck)
      		evicted->frame |= PG_REF;
		}
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    //?????
    pgtbl_entry_t *pte = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);
    p = pte + PGTBL_INDEX(vaddr);


	// Check if p is valid or not, on swap or not, and handle appropriately
    if ((p->frame & PG_VALID) == 1) {
        hit_count += 1;
    }
    //not valid not on swap
    if ((p->frame & PG_VALID) == 0 && (p->frame & PG_ONSWAP) == 0) {
        int frame = allocate_frame(p);
        init_frame(frame, vaddr);
        p->frame = frame << PAGE_SHIFT;
        // if not on swap and not valid then first reference - so set dirty bit to make sure
        // p will make to swap after first eviction (from piazza)
        p->frame |= PG_DIRTY;
        miss_count += 1;
    }
    //not valid but on swap
    if ((p->frame & PG_VALID) == 0 && (p->frame & PG_ONSWAP) == 1) {
        int frame = allocate_frame(p);
        int swap_off = swap_pagein(frame, p->swap_off);
        if (swap_off != 0) {
            return NULL;
        }
        p->frame = frame << PAGE_SHIFT;
