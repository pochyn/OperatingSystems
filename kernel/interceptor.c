...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	//if table[reg.ax].monitored == 0 (not moniotring any PIDs) we just log the original call without log_message()
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	//checking if syscall valid. Several if statements made on purpose, to make it clearer what I am checking.
	if (syscall < 0){
		return -EINVAL;
	}
	if (syscall > NR_syscalls - 1){
		return -EINVAL;
	}
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT){
		...
		//setting up default values in our my_table of syscalls
		spin_lock(&my_table_lock);
		...
		//raplace syscall with our interceptor call
	...
	if (cmd == REQUEST_SYSCALL_RELEASE){
		...
		//alongside with putting default values for monitored and intercepted
		//in my_table, need to destroy monitored list
		spin_lock(&my_table_lock);
	...
	if (cmd == REQUEST_START_MONITORING){
		//several if statements left on purpose to be clear what I am checking
		if (pid < 0){
			return -EINVAL;
		}
		...
		if (pid == 0) {
			//destory syscall list, since no need to keep track of all PIDS if monitored == 2
			spin_lock(&my_table_lock);
			table[syscall].monitored = 2;
			destroy_list(syscall);
			spin_unlock(&my_table_lock);
		} else {
			// add syscall to monitored
			spin_lock(&my_table_lock);
...
static int init_function(void) {
	int sysc = 0;
	//save original for exit_function to restore
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	orig_exit_group = sys_call_table[__NR_exit_group];

	//change syscall with our new syscall and our exit call
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long) sys_call_table);
	...
	//add all syscall to our my_table
	while (sysc < NR_syscalls) {
		spin_lock(&my_table_lock);
...
static void exit_function(void) {  
	int sysc = 0;
	//restore all intercepted syscalls and delete monitored lists
	while (sysc < NR_syscalls) {
		spin_lock(&my_table_lock);
		if (table[sysc].intercepted == 1){
			spin_lock(&sys_call_table_lock);
			set_addr_rw((unsigned long) sys_call_table);
			sys_call_table[sysc] = table[sysc].f;
			set_addr_ro((unsigned long) sys_call_table);
			spin_unlock(&sys_call_table_lock);
			table[sysc].intercepted = 0;
		}
	...
	//change to original syscall and original exit call in kernel
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long) sys_call_table);
